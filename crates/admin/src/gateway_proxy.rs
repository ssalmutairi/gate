...
/// Reverse proxy handler that forwards `/gateway/*` requests to the proxy server.
/// This enables the dashboard's "Try It" panel to work when served from the admin server.
pub async fn proxy_to_gateway(req: Request) -> Response {
    ...
    // Strip /gateway prefix to get the actual path
